    ...
    public function surveys()
    {
        ...
        if (strtoupper($this->input->method()) === 'POST') {
            ...
            if ($entity === 'survey') {
                ...
                $assignmentConflicts = $this->ipak->question_assignment_conflicts($questionIds, $surveyId);
                if ($assignmentConflicts) {
                    $conflictLabels = [];
                    foreach ($assignmentConflicts as $conflict) {
                        $conflictLabels[] = $conflict['question_code'] . ' pada ' . $conflict['survey_name'];
                    }
                    $errors[] = 'Pertanyaan sudah digunakan oleh survei lain: ' . implode(', ', $conflictLabels) . '.';
                }
    ...
    public function create_form()
    {
        ...
        if (strtoupper($this->input->method()) === 'POST') {
            ...
            $questionIds = is_array($questionIds)
                ...
                : [];
            $conflicts = $this->ipak->question_assignment_conflicts($questionIds, 0);
            if ($conflicts) {
                $errors[] = 'Salah satu pertanyaan yang dipilih sudah digunakan survei lain. Muat ulang halaman dan pilih kembali.';
            }
        ...
        $questions = $this->ipak->get_questions(false);
        $assignments = $this->ipak->get_question_assignments();
        foreach (array_keys($questions) as $questionId) {
            if (isset($assignments[$questionId])) {
